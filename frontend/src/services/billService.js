...
export async function getSuggestions(itemIds) {
  // itemIds is array of strings
  const params = new URLSearchParams();
  itemIds.forEach(id => params.append('items', id));

  const response = await api.get(`/api/bills/suggestions?${params.toString()}`);
  return response.data;
}

export async function trainRecommendations() {
  const response = await api.post(`/api/bills/train-recommendations`);
  return response.data;
}